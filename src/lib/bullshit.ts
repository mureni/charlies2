...
const DATA_FILE = "../../data/hippy.json";
...
class Bullshit {
   ...
   public static load(filename: string = DATA_FILE): boolean | Error {
      ...
      if (!existsSync(trueFile)) return new Error(`Unable to load bullshit data file '${filename}': file does not exist.`);
      const data = JSON.parse(readFileSync(trueFile, "utf8"));
